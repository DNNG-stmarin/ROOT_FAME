...
void fragFiss::PostAttenuation(int iterationPost)
{
  ...
  for(int m = 1; m <= N_BINS_MASS_TH; m++)
  {
    ...
    if(h2_mKEAtt1->GetEntries() < MIN_COUNT_MASS_ATT) continue;
    ...
    depMass1 = p1_massAtt1[numMass1]->Fit(f_massAttProj1, "QS", "", MIN_INV_ANG_FIT_TIP, MAX_INV_ANG_FIT_TIP);
  ...
  for(int m = 1; m <= N_BINS_MASS_TH; m++)
  {
    ...
    if(h2_mKEAtt2->GetEntries() < MIN_COUNT_MASS_ATT) continue;
    ...
    depMass2 = p1_massAtt2[numMass2]->Fit(f_massAttProj2, "QS", "", MIN_INV_ANG_FIT_TIP, MAX_INV_ANG_FIT_TIP);
