...
void fragFiss::PostChargeCorr(int iterationPost)
{

  // cout << "Posteriori charge corrections" << endl;
  fragDiagnostics->cd();
  TH3D* h3_anodeMassGrid1 = new TH3D("h3_anodeMassGrid1_"+ TString(to_string(iterationPost)),"h3_anodeMassGrid1;gph/aph;aph;mass",N_BINS_RATIO,0,ALL_RATIO,N_BINS_APH,0,MAX_APH, N_BINS_MASS_TH, MIN_MASS, MAX_MASS);
  TH3D* h3_anodeMassGrid2 = new TH3D("h3_anodeMassGrid2_"+ TString(to_string(iterationPost)),"h3_anodeMassGrid2;gph/aph;aph;mass",N_BINS_RATIO,0,ALL_RATIO,N_BINS_APH,0,MAX_APH, N_BINS_MASS_TH, MIN_MASS, MAX_MASS);
  ...
  const int MIN_ANGLE_BIN = (int)(MIN_ANG1/MAX_ANG1*N_BINS_RATIO);
  ...
  // cout << "midMass: " << MID_MASS_BIN << endl;
  ...
  */
   ...
   for (int b=0; b < N_BINS_MASS_TH; b++)
   {
     ...
     for (int i = MIN_ANGLE_BIN; i < N_BINS_ANGLE; i++)
     {
       cosAx[i] = 0.5*(projMass1->GetBinCenter(i+1) + (projMass1->GetBinCenter(i+1)));
       angM1[i]  = projMass1->GetBinContent(i+1);
       angM2[i]  = projMass2->GetBinContent(i+2);
     ...
     //
     if (dFound1) {chargeLength1[b] = projMass1->GetBinCenter(dMinBinangM1+1);}
     else {chargeLength1[b] = chargeLength1[b-1];}

     if (dFound2) {chargeLength2[b] = projMass2->GetBinCenter(dMinBinangM2+1);}
