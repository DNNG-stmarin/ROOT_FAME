...
void fragFiss::PostAngle(int iterationPost)
{
  ...
  const int MIN_ANGLE_BIN = (int)(MIN_ANG1/MAX_ANG1*N_BINS_ANGLE);
  ...
  */
   ...
   for (int b=0; b < N_BINS_MASS_TH; b++)
   {
     ...
     for (int i = MIN_ANGLE_BIN; i < MAX_ANGLE_POST_BIN; i++)
     {
     ...
     // derivative filter
     for(int i = MIN_ANGLE_BIN; i < MAX_ANGLE_POST_BIN - 1; i++)
     {
     ...
     for(int i = MIN_ANGLE_BIN; i < MAX_ANGLE_POST_BIN - 2; i++)
     {
       // calculate local derivative
     ...
     if (dFound1)
     {
       ...
       double slope1 = (d2angM1[iterEdge1+1] - d2angM1[iterEdge1])/(projMass1->GetBinCenter(iterEdge1+1) - projMass1->GetBinCenter(iterEdge1));

       double crossPoint1 = projMass1->GetBinCenter(iterEdge1) - d2angM1[iterEdge1]/slope1;
     ...
     if (dFound2)
     {
       ...
       double slope2 = (d2angM2[iterEdge2+1] - d2angM2[iterEdge2])/(projMass2->GetBinCenter(iterEdge2+1) - projMass2->GetBinCenter(iterEdge2));

       double crossPoint2 = projMass2->GetBinCenter(iterEdge2) - d2angM2[iterEdge2]/slope2;
